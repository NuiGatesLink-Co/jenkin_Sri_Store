...
- A manually voided sale with no credit note is not separately subtracted. This matches the existing client calculation, while full returns are represented by credit notes. Revisit only if the owner decides direct voids must change report semantics.
  - **Superseded by #95 (2026-09-13) for `GET /reports/summary` only:** a manual void (`POST /sales/:id/void`, added by #23 after this note) has no credit note, so counting it overstated revenue; summary now uses #30's `COUNTED_SALE` bill set for every figure, gross profit included — manual voids excluded, bills auto-voided by a full return still counted with their credit notes netted. `top-products`, `by-category` and `product-sales` were aligned by **#97** (2026-09-13): their `sale_events` CTEs use the same `COUNTED_SALE` set; credit notes need no filter because a manually voided bill cannot have one (`SALE_HAS_RETURNS` / `SALE_VOIDED`).
  - ~~Because a manual void is dated by its bill, voiding an old bill changes that past range's summary retroactively — the same class as **#94** for closing reports.~~ **Resolved by #94 (2026-09-13, owner's option A):** `POST /sales/:id/void` is refused unless the bill's `shift_id` is the calling device's open shift (`409 NO_OPEN_SHIFT` / `409 SALE_NOT_IN_OPEN_SHIFT`), so only a bill from the drawer that is still open can be manually voided; an older bill is undone by a credit note dated and shifted into the current drawer. A same-shift void still moves the summary range of its bill's date — for a shift open across midnight that is yesterday's, or any earlier day while that shift is still unclosed and unarchived — but never a closed shift's. The credit-note route needs a drawer too: after today's close a cash credit note is refused loudly with `409 NO_OPEN_SHIFT` instead of landing, stamped `shift_id` null, in no report (#100, owner's decision 2026-09-13 — cash only; `โอน`/`หักจากเครดิต` are still taken and stamped null, and move no drawer cash).
