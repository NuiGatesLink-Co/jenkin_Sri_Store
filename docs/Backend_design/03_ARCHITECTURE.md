...
> ℹ️ **2026-09-08:** ข้อเสนอ CouchDB (= ปลุก Architecture B กลับมาโดย sync engine มากับ DB) ถูก**ปฏิเสธ**
> วันเดียวกัน — [ADR-0012](adr/0012-couchdb-replaces-postgres.md) · ข้อสรุป §7 และ Gantt §8 เป็นของจริงตามเดิม
...
> * **scarcity rule / `offlineOk` ถูกยกเลิก (D3)** — ออฟไลน์ขายได้ถ้าสต็อกในเครื่อง `pos` พอ เพราะ `pos` เครื่องเดียวเป็นผู้ขายคนเดียว (ADR-0004)
>   ความขัดแย้งเหลือแค่ที่ `backoffice` แก้ระหว่างออฟไลน์ → reconciliation · ป้ายเทาไม่มีแล้ว
> * **ตัวกระตุ้น Degraded (D5):** health check ล้ม 3 ครั้ง / ช้า > 5 วินาที **หรือ write ที่ server ไม่ตอบ** · ออกจาก Degraded ด้วย **health check เท่านั้น**
>   (state diagram ด้านล่างมีแค่ข้อแรก และ "Conflict → Online: ผู้จัดการเคลียร์" ไม่ตรงแล้ว — ไม่มีผู้จัดการ D12 และ Conflict ไม่หยุดขาย · ดู 08 §4)
> * สิ่งที่ห้ามทำตอน Degraded ถูกแทนด้วยรายการ op ใน 08 §5 · วงเงินเครดิตตอนออฟไลน์ยังไม่เคาะ (08 §15 Q3)
> * `sales.sync_status` ไม่ทำ — สถานะอยู่ใน outbox (08 §5.3)
...
~~**กลไก "scarcity rule" — หัวใจของแบบนี้**~~ — **ยกเลิก 2026-09-15 (#240 D3)** ข้อความถึงก่อนหัวข้อ *ทำไมไม่ใช้ "จองโควตาสต็อก"* เก็บไว้เป็นประวัติ · ดู [`08 §5`](08_PHASE2_SPEC.md)
...
และต้องมี ~~`sales.sync_status ('local' | 'confirmed' | 'rejected')`~~ *(สถานะอยู่ใน outbox — 08 §5.3)* + **คิวให้เจ้าของร้านเคลียร์บิล
...
* วงเงินเครดิตช่างเป็นตัวเลขที่ enforce ตอนออฟไลน์ไม่ได้ ~~(สองเครื่องแก้พร้อมกันได้)~~ *(ไม่จริงภายใต้ `pos` เครื่องเดียว — เหลือแค่ `backoffice` แก้วงเงิน · วิธีจัดการยังไม่เคาะ 08 §15 Q3)*
...
* ~~🔴 **ต้องเลือก production host ก่อนงาน `q4` (cutover)** — VM คณะไม่ใช่ที่ที่ POS ของร้าน
  ...
  **เคาะแล้ว 2026-09-15 (#240 D15):** production = VM ของภาค `mob04` สภาพแวดล้อมเดียว เป้าเฟส 2 = รันในมหาวิทยาลัยพร้อมส่งอาจารย์ ·
  **cutover ร้านจริงจากนอกมหาวิทยาลัยย้ายไปเฟสถัดไป** (#231) · เหตุผลเดิมข้างบนยังจริงสำหรับเฟสนั้น · ดู [`08 §14`](08_PHASE2_SPEC.md)
