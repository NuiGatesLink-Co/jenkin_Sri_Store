...
| หัวข้อ | ค่าที่เคาะ |
|---|---|
| credential | **PIN ออฟไลน์แยก** ต่อ user ต่อเครื่อง — ไม่ใช่ `users.pin_hash` · พนักงานตั้งตอน**ออนไลน์**บนเครื่อง `pos` เท่านั้น · เก็บเป็น hash แบบช้า (WebCrypto, salt สุ่มต่อ user ต่อเครื่อง) ใน Drift ของเครื่องนั้น · **ไม่เคยส่งขึ้น server** |
| ใครใช้ได้ | user ที่**ล็อกอินออนไลน์สำเร็จบนเครื่องนี้ภายใน 7 วัน** (นับถึงเวลาที่ใช้ PIN) · เครื่อง `role='pos'` เท่านั้น (ADR-0004) |
| ใช้ได้เมื่อไหร่ | เฉพาะตอน **Degraded** (`03 §4`) — ถ้าออนไลน์ต้องล็อกอินจริงเสมอ |
| ทำอะไรได้ | เฉพาะที่ degraded mode อนุญาตอยู่แล้ว (`03 §4`) — ไม่มีสิทธิ์เพิ่ม ไม่ออก token |
| ตรวจซ้ำตอน push | ทุก operation ใน outbox ติด `userId` + โหมดยืนยันตัวตน `offline_pin` · `/sync/push` ตรวจ: user ยัง active, ล็อกอินออนไลน์ล่าสุดบนเครื่องนั้นไม่เกิน 7 วันก่อนเวลาของ operation, เครื่องไม่ถูก retire · ไม่ผ่าน = `rejected` เข้าคิว reconciliation (ไม่ทิ้งเงียบ — บิลพิมพ์ไปแล้ว) · เขียน `audit_log` |
| ใส่ผิด | ล็อกในแอปหลังผิดติดกันหลายครั้ง — **กันได้แค่คนทั่วไปที่หน้าเคาน์เตอร์** ไม่กันคนที่ได้ storage |

**threat model (ยอมรับ):** คนที่ได้ storage ของเครื่องแล้วแกะ PIN ออฟไลน์ได้ จะได้แค่สิทธิ์ degraded mode
**บนเครื่องที่ตัวเองถืออยู่แล้ว** ไม่ได้ credential ออนไลน์ใด ๆ และทุกบิลยังถูกตรวจซ้ำตอน push
พนักงานที่ถูกปิดบัญชีใช้ PIN ออฟไลน์ต่อได้ไม่เกิน 7 วัน และบิลช่วงนั้นเข้าคิว reconciliation
...
* [x] ~~เปิดร้านตอนเน็ตล่มหลัง token ตาย~~ — **เคาะแล้ว 2026-09-15 (#187):** PIN ออฟไลน์แยก, ใช้ได้ภายใน 7 วัน
      นับจากล็อกอินออนไลน์ล่าสุดบนเครื่องนั้น (หัวข้อ *"การเข้าระบบออฟไลน์ของเครื่อง `pos`"*)
