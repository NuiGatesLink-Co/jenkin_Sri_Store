...
> — `POST /sales` ลดจาก ~19 เหลือ ~16.5 ms · **`tx.5` (#154) ย้ายการเช็ค PIN ออกไปก่อน
> `runIdempotent` แล้ว:** ทรานแซกชันที่ยาวที่สุดของ void เหลือ ~14–22 ms และ latency เลิกเป็นขั้นบันได
>
...
* 🔴 **`runTx` ต้อง *ไม่* รับ `tid` เป็นพารามิเตอร์** — คลาส `TenantService` เดิมเขียนเป็น
  `runTx(tid, fn)` (ตอนนี้เป็น `runTx(fn)` แล้ว ตั้งแต่ #150) ซึ่ง **ผิดและห้ามใช้รูปนั้น**: call site ไหน
