...
>
> สถานะ 2026-09-10: **มี prototype ที่รันได้ทั้งระบบแล้ว** (ทำบน worktree แยก — ภายหลัง commit เป็น `0feaf94`)
> — typecheck + lint สะอาด, unit 77 ผ่าน, e2e 105 ผ่าน, full-suite รันซ้ำ 8 รอบติดไม่แดง
>
> **สถานะ 2026-09-14:** แตกเป็น issue แล้ว สไลซ์ละใบ ต่อกันเป็นสายเดียว —
> #149 `tx.0` → #150 `tx.1` → #151 `tx.2` → #152 `tx.3` → #153 `tx.4` → #154 `tx.5` (parent #142)
> ⚠️ prototype (`0feaf94` บน `worktree-agent-a1756ff02f223b4eb`, **ห้าม merge**) และตัวเลขข้างบน
> **มาก่อน #22 / #24 / #30 และ Lane B** — วันนี้มีราว 20 ไฟล์ที่อ่าน `currentRequestContext()`
> ไม่ใช่ 4 service ที่ `tx.2` ระบุ และ `IdempotencyInterceptor` คุม write ที่ไม่ใช่เงินด้วย
> ขอบเขตของ `tx.2` / `tx.3` จึงต้องเคาะใน #151 / #152 ก่อนลงมือ ตัวเลขใน §2 อย่าเชื่อตามตัวอักษร
> ~~⚠️ §2 `tx.0` ด้านล่างยังอ้างย่อหน้า `CLAUDE.md` ที่ถูกเขียนใหม่ไปแล้ว และ AC2 ของมันขัดกับ
> หมายเหตุวันที่มีผลใน ADR-0003 — ยึด ADR: `server/README.md` อธิบาย split เดิมว่า
> **ยังบังคับใช้จนถึง `tx.4`** ไม่ใช่ว่าหายไปแล้ว (ทำใน #149)~~ — **ล้าสมัยแล้ว (superseded)**
> ตั้งแต่ `tx.4` (#153): split เดิมถูกถอดออก และ `server/README.md` อธิบายกลไกใหม่ที่บังคับใช้อยู่
>
> ~~**สถานะ 2026-09-14 (#153):** `tx.0`–`tx.4` ลงแล้ว (#149–#153) addendum ของ ADR-0003 เป็น
...
> argon2 ออกไปด้วย) — ตัวเลขนั้นเป็นของ `tx.5` ดูคอมเมนต์ใน #154~~ — **ล้าสมัยแล้ว (superseded)
> โดยสถานะ 2026-09-15 ด้านล่าง**
>
...
> (ขั้นบันได) → ~115/121/127/133 ms — AC3 ผ่าน (เลิกเป็นขั้นบันได) แต่ตัวเลขไม่ใช่ 142/143/157/157
> ที่ §2 คาดไว้ ซึ่งมาจาก prototype บนเครื่องอื่น · รอบรีวิวพบว่า lockout ของ PIN ต้องเป็น
> `consumeAttempt` (atomic) เมื่อ argon2 ออกจากทรานแซกชัน — ดู `server/README.md` · ผลข้างเคียง: key ที่ done แล้วไม่ข้ามการเช็ค PIN อีก —
