...
| [0013](0013-cicd-toolchain.md) | toolchain CI/CD + deploy | GitHub Actions (คงเดิม ไม่ Jenkins) + Trivy สแกน image บล็อก push + **GHCR** public (server + web เป็น image ทั้งคู่) + **Ansible** deploy อัตโนมัติขึ้น VM คณะ (`demo`) + **etcd** เฉพาะ dynamic config ไม่ใช่ข้อมูลธุรกิจ + **Monitoring (Node Exporter + Prometheus + Grafana)** ผูก loopback · เอกสารเจ้าของ = `07_CICD_DEPLOY.md` · addendum **2026-09-15** *"Actions เข้าถึง VM อย่างไร"*: VM อยู่ในเครือข่ายมหาวิทยาลัย → deploy ผ่าน **self-hosted runner บน VM** (ขาออกอย่างเดียว, user `gha-runner` ไม่มี docker, สั่งได้แค่ wrapper `pos-deploy` ที่รับเฉพาะ commit บน `main` + job-started hook) รัน playbook เดิมแบบ local, ปฏิเสธ port forward / Tailscale (#67) | Accepted 2026-09-10 |
