...
| [0003](0003-tenant-lifecycle.md) | ร้านถูกระงับ/เลิกใช้ | `tenants.status` และบังคับที่ **guard** (ไม่ `SET LOCAL` → RLS คืน 0 แถวเอง) ไม่ใส่ใน RLS predicate · addendum **2026-09-10** *"ใครตัดสิน กับ ใครลงมือ"*: guard เช็คสถานะแล้ว**ตั้งชื่อ tenant ลง scope** ส่วน `TenantService.runTx` เปิดทรานแซกชัน+`set_config` ใน handler — **Accepted, มีผลตั้งแต่ 2026-09-14** (`tx.4` #153 ของ [แผนย้าย](0003-handler-scoped-migration-plan.md)) แทนรูป middleware → guard → interceptor ของ #75 | Accepted |
| [0004](0004-device-roles.md) | กี่เครื่องต่อร้าน | ไม่นับจำนวนเครื่อง แต่แบ่ง `role`: `pos` **1 เครื่อง** (แตะลิ้นชัก+ออกเลข+เขียน offline), `backoffice` กี่เครื่องก็ได้ (สต็อก/รายงาน, ออนไลน์เท่านั้น) · **เครื่อง = device token ที่ owner enrol ให้** (`/devices`, `/auth/device`) ไม่ใช่ id จาก client · addendum **2026-09-15 (#240)**: ไม่มี `offlineOk` (D3), แท็บเดียว (D10), `/sync/push` ยืนยันด้วย device token (D8) · **รอบ 2**: device role ไม่เปลี่ยน, role คนเหลือ `owner` (E1) · **รอบ 3**: F6/F7/F8 — [`08`](../08_PHASE2_SPEC.md) | Accepted |
...
| [0007](0007-receipt-numbering.md) | รูปแบบเลขที่ใบเสร็จ | `RC01-2569-08-0042` เรียงต่อเครื่อง รีเซ็ตรายเดือน · **เฟส 1 server ออกทุกเลข** เฟส 2 เครื่อง `pos` ออก RC/CN เอง ส่วน PO/QT/CP server ออกตลอด · addendum **2026-09-15 (#240 D4)**: `pos` ออก RC/CN **ทั้งออนไลน์และออฟไลน์** server ไม่ออกให้ `pos` อีก · **รอบ 2**: ขึ้นเดือนใหม่ออฟไลน์เริ่ม `0001` (E8), วันที่บิล = นาฬิกาเครื่อง (E9), high-water mark เป็น upsert — [`08 §9`](../08_PHASE2_SPEC.md) | Accepted |
...
| [0009](0009-jwt-session-lifetime.md) | อายุ JWT + การเซ็น | ล็อกอินใหม่ทุกวัน — access 15 นาที + refresh หมดอายุ **ตี 4** ไม่ใช่ 24 ชม.นับจากล็อกอิน · **RS256 + `kid`** (addendum 2026-09-09) · addendum **2026-09-15 (#240)**: ~~role `owner`+`staff` (D12), PIN ออฟไลน์ทุกคนแต่สิทธิ์ staff + owner ห้ามซ้ำ PIN ออนไลน์ (D11/D13)~~, เน็ตกลับขายต่อ ส่งด้วย device token (D8) · **รอบ 2**: role `owner` เดียว + บัญชีร้านเดียว (E1/E2), void ไม่มี PIN (E3), 1 PIN ออฟไลน์ต่อเครื่องไม่ซ้ำรหัสผ่าน (E5) · **รอบ 3**: ผู้กระทำ push = user active คนเดียว (F3), 3 วันตรวจที่เครื่องเท่านั้น (F5), retire/enrol/export ต้องมี device token (F6) — [`08 §13`](../08_PHASE2_SPEC.md) | Accepted |
| [0010](0010-client-write-through-cache.md) | client ใช้ Drift ยังไง | `ApiRepository` = implementation ใหม่ของ interface เดิม เขียนผลลง Drift (**write-through**) และ **map ที่ชั้น repository** ไม่ regenerate schema ตาม Postgres · addendum **2026-09-15 (#240)**: outbox ตารางเดียว, ตัวกระตุ้น Degraded (D5), ~~catalogue เข้าคิว/ออนไลน์เท่านั้น (D9)~~, `offlineOk` ไม่ใช้ (D3) · **รอบ 2**: รายการ op (E6), ลบ `offlineOk` (E10), cursor = `meta.nextCursor` ของ server — [`08`](../08_PHASE2_SPEC.md) | Accepted |
...
| [0013](0013-cicd-toolchain.md) | toolchain CI/CD + deploy | GitHub Actions (คงเดิม ไม่ Jenkins) + Trivy สแกน image บล็อก push + **GHCR** public (server + web เป็น image ทั้งคู่) + **Ansible** deploy อัตโนมัติขึ้น VM คณะ (`demo`) + **etcd** เฉพาะ dynamic config ไม่ใช่ข้อมูลธุรกิจ + **Monitoring (Node Exporter + Prometheus + Grafana)** ผูก loopback · เอกสารเจ้าของ = `07_CICD_DEPLOY.md` · addendum **2026-09-15**: `mob04` = production เดียว (#242), ~~deploy ด้วย self-hosted runner บน `mob04` (E11)~~ **deploy แบบ pull จาก `mob04` (F4 รอบ 3)** | Accepted 2026-09-10 |
...
| 0003 | guard ทำทั้ง status check และ `SET LOCAL` (ไม่มี interceptor แยก); cache miss/Redis ล่ม → อ่าน Postgres ห้าม fail-open/closed; key status ต้องมี TTL · **2026-09-10:** เพิ่มหัวข้อ *"ใครตัดสิน กับ ใครลงมือ"* + [แผนย้าย `tx.0`–`tx.5`](0003-handler-scoped-migration-plan.md) · **2026-09-14:** addendum **Accepted** เมื่อ `tx.4` (#153) ลง — ถอด split middleware → guard → interceptor ของ PR #75 ออก |
...
* ~~**เกณฑ์ `offlineOk`** — ต้องคำนวณจากข้อมูลขายจริง (ไฟล์ backup `sa_*`) ก่อนงาน `q2`~~ — **ไม่ต้องมีแล้ว 2026-09-15 (#240 D3)**
* **คำไทย 3 ตัวที่ขึ้นหน้าร้าน** — `DEVICE_ROLE_FORBIDDEN` / `TENANT_SUSPENDED` /
  ~~`OFFLINE_NOT_ALLOWED`~~ (หมดความหมาย D3) ต้องให้คนขายอ่านแล้วเลือกคำเอง
* ~~**production host** — ก่อนงาน `q4`~~ — **เคาะ 2026-09-15 (#242): `mob04`** สภาพแวดล้อมเดียว · cutover ร้านจริง = เฟสถัดไป
* **เฟส 2 (2026-09-15):** คำถามที่ยังต้องให้เจ้าของตอบรวมไว้ที่ [`08_PHASE2_SPEC.md §18`](../08_PHASE2_SPEC.md) (รอบ 3 เหลือแค่ ticket ข้อความไทย F10)
