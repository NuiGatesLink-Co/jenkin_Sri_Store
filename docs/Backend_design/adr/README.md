...
| [0004](0004-device-roles.md) | กี่เครื่องต่อร้าน | ไม่นับจำนวนเครื่อง แต่แบ่ง `role`: `pos` **1 เครื่อง** (แตะลิ้นชัก+ออกเลข+เขียน offline), `backoffice` กี่เครื่องก็ได้ (สต็อก/รายงาน, ออนไลน์เท่านั้น) · **เครื่อง = device token ที่ owner enrol ให้** (`/devices`, `/auth/device`) ไม่ใช่ id จาก client · addendum **2026-09-15 (#240)**: ไม่มี `offlineOk` (D3), แท็บเดียว (D10), `/sync/push` ยืนยันด้วย device token (D8) — [`08`](../08_PHASE2_SPEC.md) | Accepted |
...
* ~~**production host** — ก่อนงาน `q4`~~ — **เคาะ 2026-09-15 (#240 D15): `mob04`** สภาพแวดล้อมเดียว · cutover ร้านจริง = เฟสถัดไป
* **เฟส 2 (2026-09-15):** คำถามที่ยังต้องให้เจ้าของตอบรวมไว้ที่ [`08_PHASE2_SPEC.md §15`](../08_PHASE2_SPEC.md) (Q1–Q14)
