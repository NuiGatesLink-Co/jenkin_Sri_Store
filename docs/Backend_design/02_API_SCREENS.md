...
| 409 | `NO_OPEN_SHIFT` | `No open shift` (ลิ้นชัก/ปิดกะ และตั้งแต่ 2026-09-13 `POST /sales` + `POST /mechanics/:id/credit-payments` ด้วย — ไม่มีกะเปิด = ไม่รับเงิน · #94: `POST /sales/:id/void` ด้วย) |
...
| 409 | `SALE_NOT_IN_OPEN_SHIFT` | 🔴 **ยังไม่ร่าง — ต้องให้เจ้าของร้านเป็นคนตั้ง** | `POST /sales/:id/void` กับบิลที่ `shift_id` ไม่ใช่กะที่เปิดอยู่ของเครื่องนี้ (กะที่ปิดแล้ว, กะของเครื่องอื่น, หรือบิลนำเข้าที่ไม่มีกะ) — ถ้าปล่อยให้ void รายงานปิดกะที่นับเงินไปแล้วจะเปลี่ยนย้อนหลัง และลิ้นชักที่เงินออกจริงไม่มีรายการเงินออก (#94) · ทางที่ถูกคือออกใบลดหนี้ ซึ่งลงกะปัจจุบัน (ต้องมีกะเปิดอยู่ — ไม่งั้นใบลดหนี้ได้ `shift_id` null ดู #100) · ของเดิมไม่มีปุ่ม void จึงไม่มีเคสนี้ · **#94 คืนข้อความอังกฤษไว้ก่อน** ไม่แต่งไทยเอง · ข้อความควรบอกให้ไปออกใบลดหนี้ |
