...
เจ้าของโปรเจกต์เลือก **แบบ "ยังไม่ยืนยัน"** (ไม่ใช่แบบ "ถือว่าขายแล้ว กำลังอัปเดตไปยังเซิร์ฟเวอร์" — แบบนั้นเป็นของ phase 2
ที่มี outbox + หน้า reconciliation รองรับกรณี server ปฏิเสธทีหลัง ใน phase 1 ถ้าใช้จะมีกรณีของออกจากร้านแต่บิลไม่มีจริง):

| หน้าจอ | ข้อความไทย | พฤติกรรม |
|---|---|---|
| Checkout — ระหว่างไม่ทราบผล | `กำลังยืนยันการขายกับเซิร์ฟเวอร์ อย่าเพิ่งส่งของให้ลูกค้า` — ใจความเจ้าของโปรเจกต์เลือก 2026-09-15 ยังไม่ผ่านคนหน้าร้าน | **ล็อกตะกร้า** (แก้/ล้าง/เปลี่ยนวิธีจ่ายไม่ได้) และส่งบิลเดิมซ้ำด้วย id + `Idempotency-Key` เดิม (`PendingWrites`) จนกว่า server ตอบ |
| Checkout — server ตอบรับ (2xx รวม replay) | ใช้หน้าจอขายสำเร็จ/ใบเสร็จเดิม | ปลดล็อก ล้างตะกร้า |
| Checkout — server ปฏิเสธ (4xx = คำตัดสิน) | ข้อความของ code นั้นตามตารางด้านบน/`db.js` | ปลดล็อก ตะกร้าคงเดิมให้แก้ได้ |

ยังไม่ตัดสิน (อยู่ใน ticket implement): ปุ่มยกเลิกเองเมื่อ server ไม่ตอบนาน (การยกเลิกขณะไม่ทราบผลคือความเสี่ยงบิลซ้ำเดิม)
และหน้า Returns / Cash Drawer ใช้รูปแบบเดียวกันหรือไม่ (หน้าชำระหนี้ช่างเป็น outbox อยู่แล้ว — #24)
